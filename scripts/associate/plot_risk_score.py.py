...
try:
    ...
    snakemake = load_rule_args(
        ...
        rule_name = 'associate__plot_risk_score',
        default_wildcards={
...
path = snakemake.params["output_basedir"] + f"/predictions_cloud"
...
path = snakemake.params["output_basedir"] + f"/predictions_vs_common_PRS"
pn.ggsave(plot, path + ".png", dpi=DPI)
pn.ggsave(plot, path + ".pdf", dpi=DPI)
plot_df.to_parquet(path + ".parquet", index=False)
display(plot)
...
plot = (
    ...
    + pn.ggtitle(f"Precision-Recall curves of LGBMRegressor models predicting extreme {phenotype_col}")
    + pn.theme(title = pn.element_text(va = "top", linespacing = 4))
    + pn.coord_equal()
)
...
plot = (
    ...
    + pn.ggtitle(f"auPRC of LGBM models prediction extreme {phenotype_col}")
